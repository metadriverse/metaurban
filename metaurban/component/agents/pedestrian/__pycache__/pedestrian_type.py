...
class SimplePedestrian(BasePedestrian):
    ...
    @property
    def ACTOR_ROLL(self):
        if not hasattr(self, 'random_actor'): self.random_actor = AssetPaths.Pedestrian.get_random_actor() 
...
class StaticPedestrian(BasePedestrian):
    ...
    def HEIGHT(self):
        if not hasattr(self, 'random_static_actor'):
            if self.engine.global_config['vis_bedlam_motion']: demo_ver = 1
            elif self.engine.global_config['fixed_standing_demo']: demo_ver = 3
            elif self.engine.global_config['use_customized_camera_traj']: demo_ver = 4
            else: demo_ver = 0
            self.random_static_actor = AssetPaths.Pedestrian.get_static_random_actor(demo_ver=demo_ver) #self.MAX_ACTOR_NUM)
    ...
    @property
    def ACTOR_ROLL(self):
        if not hasattr(self, 'random_static_actor'): self.random_static_actor = AssetPaths.Pedestrian.get_static_random_actor() 

...
from metaurban.base_class.base_runnable import BaseRunnable
from metaurban.constants import CamMask
from metaurban.constants import MapTerrainSemanticColor, MetaUrbanType, PGDrivableAreaProperty
from metaurban.utils.shapely_utils.geom import find_longest_edge
    ...
    def get_semantic_map(
        ...
        if self._semantic_map is None:
            ...
            for obj in all_lanes.values():
                if MetaUrbanType.is_lane(obj["type"]) and "lane" in layer:
                    polygons.append((obj["polygon"], MapTerrainSemanticColor.get_color(obj["type"])))
                elif "lane_line" in layer and (MetaUrbanType.is_road_line(obj["type"])
                                               or MetaUrbanType.is_road_boundary_line(obj["type"])):
                    if MetaUrbanType.is_broken_line(obj["type"]):
            ...
            mask[..., 0] = color_setting.get_color(MetaUrbanType.GROUND)
            ...
            if "crosswalk" in layer:
                for id, sidewalk in self.crosswalks.items():
                    ...
                    angle = angle / 1000 + MapTerrainSemanticColor.get_color(MetaUrbanType.CROSSWALK)

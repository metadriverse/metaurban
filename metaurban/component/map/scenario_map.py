...
from metaurban.component.scenario_block.scenario_block import ScenarioBlock
...
class ScenarioMap(BaseMap):
        ...
        for lane_id, data in self.blocks[-1].map_data.items():
            ...
            if MetaUrbanType.is_road_line(type):
                ...
                if MetaUrbanType.is_broken_line(type):
                    ret[map_feat_id] = {
                        "type": MetaUrbanType.LINE_BROKEN_SINGLE_YELLOW
                        if MetaUrbanType.is_yellow_line(type) else MetaUrbanType.LINE_BROKEN_SINGLE_WHITE,
                        "polyline": resampled
                    }
                else:
                    ret[map_feat_id] = {
                        "polyline": resampled,
                        "type": MetaUrbanType.LINE_SOLID_SINGLE_YELLOW
                        if MetaUrbanType.is_yellow_line(type) else MetaUrbanType.LINE_SOLID_SINGLE_WHITE
                    }
            elif MetaUrbanType.is_road_boundary_line(type):
                line = np.asarray(data[ScenarioDescription.POLYLINE])[..., :2]
                length = get_polyline_length(line)
                resampled = resample_polyline(line, interval) if length > interval * 2 else line
                ret[map_feat_id] = {"polyline": resampled, "type": MetaUrbanType.BOUNDARY_LINE}
    ...
    #             # Discard the data for those map features that are not reconstructed by metaurban.
    #             pass
    #
    ...
    #             #     map_features[map_feat_key]["polyline"] = convert_polyline_to_metaurban(
    ...
    #             #         map_features[map_feat_key]["position"] = waymo_to_metaurban_vector(old_map_feat["position"])
    #             #     else:
...
if __name__ == "__main__":
    ...
    from metaurban.manager.scenario_data_manager import ScenarioDataManager

...
class BaseTrafficLight(BaseObject):
    ...
    def __init__(
        ...
        self.set_metaurban_type(MetaUrbanType.TRAFFIC_LIGHT)
        self.lane = lane
    ...
    def top_down_color(self):
        ...
        if status == MetaUrbanType.LIGHT_GREEN:
            return [0, 255, 0]
        if status == MetaUrbanType.LIGHT_RED:
            return [255, 0, 0]
        if status == MetaUrbanType.LIGHT_YELLOW:
            return [255, 255, 0]
        if status == MetaUrbanType.LIGHT_UNKNOWN:
            return [180, 180, 180]

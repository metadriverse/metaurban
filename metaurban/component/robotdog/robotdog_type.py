...
from metaurban.component.robotdog.base_robotdog import BaseRobotDog, EgoRobotDog
import platform
...
class CustomizedCar(BaseRobotDog):
...
class DefaultVehicle(BaseRobotDog):
    ...
    path = ['robot_dog-default.glb', (1, 1, 1), (0, 0.075, 0.), (0, 0, 0)]
...
class EgoVehicle(EgoRobotDog):
    ...
    FRONT_WHEELBASE = 1.05234
    REAR_WHEELBASE = 1.4166
    path = ['robot_dog-ego.glb', (1, 1, 1), (0, 0.075, 0.), (0, 0, 0)]
...
def random_robotdog_type(np_random, p=None):
    prob = [1 / len(robotdog_type) for _ in range(len(robotdog_type))] if p is None else p
    return robotdog_type[np_random.choice(list(robotdog_type.keys()), p=prob)]


robotdog_type = {"default": DefaultVehicle, 'ego': EgoVehicle}

robotdog_class_to_type = inv_map = {v: k for k, v in robotdog_type.items()}

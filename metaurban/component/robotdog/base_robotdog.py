...
class BaseRobotDog(BaseObject, BaseRobotDogState):
    """
    ...
    def __init__(
        ...
        BaseRobotDogState.__init__(self)
    ...
    def _add_visualization(self):
        if self.render:
            ...
            if path not in BaseRobotDog.model_collection:
                car_model = self.loader.loadModel(AssetLoader.file_path("models", path))
                car_model.setTwoSided(False)
                BaseRobotDog.model_collection[path] = car_model
                car_model.setScale(1.5)
                ...
                car_model = BaseRobotDog.model_collection[path]
    ...
    def destroy(self):
        super(BaseRobotDog, self).destroy()
    ...
    def set_velocity(self, direction, *args, **kwargs):
        super(BaseRobotDog, self).set_velocity(direction, *args, **kwargs)
        self.last_velocity = self.velocity
        self.last_speed = self.speed

    def set_state(self, state):
        super(BaseRobotDog, self).set_state(state)
    ...
    def set_panda_pos(self, pos):
        super(BaseRobotDog, self).set_panda_pos(pos)
        self.last_position = self.position

    def set_position(self, position, height=None):
        super(BaseRobotDog, self).set_position(position, height)
        self.last_position = self.position
    ...
    def get_state(self):
        ...
        """
        state = super(BaseRobotDog, self).get_state()
        state.update(
            {
    ...
    def __del__(self):
        super(BaseRobotDog, self).__del__()
        # self.engine = None
        self.navigation = None
        self.wheels = None
    ...
    def detach_from_world(self, physics_world):
        ...
        super(BaseRobotDog, self).detach_from_world(physics_world)
    ...
    def attach_to_world(self, parent_node_path, physics_world):
        ...
        super(BaseRobotDog, self).attach_to_world(parent_node_path, physics_world)
    ...
    def panda_color(self):
        c = super(BaseRobotDog, self).panda_color
    ...
    def convert_to_world_coordinates(self, vector, origin):
        return super(BaseRobotDog, self).convert_to_world_coordinates([-vector[-1], vector[0]], origin)

    def convert_to_local_coordinates(self, vector, origin):
        ret = super(BaseRobotDog, self).convert_to_local_coordinates(vector, origin)
        return np.array([ret[1], -ret[0]])

    @property
    def heading_theta(self):
        return wrap_to_pi(super(BaseRobotDog, self).heading_theta + np.pi / 2)
    ...
    def set_heading_theta(self, heading_theta, in_rad=True) -> None:
        ...
        """
        super(BaseRobotDog, self).set_heading_theta(heading_theta - np.pi / 2, in_rad)
        self.last_heading_dir = self.heading
...
class EgoRobotDog(BaseObject, BaseRobotDogState):
    """
    Basic Robot Dog
    """
    COLLISION_MASK = CollisionGroup.Vehicle
    ...
    def __init__(
        ...
        self.save_data = True
        if self.save_data:
            self.save_root = './il_data'
            import os
            os.makedirs(self.save_root, exist_ok=True)
            self.data_id = len(os.listdir(self.save_root))
            os.makedirs(os.path.join(self.save_root, f'collection_set_{self.data_id:06d}'), exist_ok=True)
            self.time_step = 0
    ...
    def reset(
        ...
        **kwargs
        ...
        if vehicle_config is not None:
            self.update_config(vehicle_config)
        from metaurban.component.robotdog.robotdog_type import robotdog_class_to_type
        self.config["vehicle_model"] = robotdog_class_to_type[self.__class__]
        ...
        #     else:
        #         lane = map.road_network.get_lane(self.config["spawn_lane_index"])
        #         position = lane.position(self.config["spawn_longitude"], self.config["spawn_lateral"])#[self.config["spawn_longitude"],  -(PGDrivableAreaProperty.SIDEWALK_NEAR_ROAD_WIDTH + PGDrivableAreaProperty.SIDEWALK_WIDTH / 2)]
        #         heading = lane.heading_theta_at(self.config["spawn_longitude"])
        # else:
        #     assert self.config["spawn_position_heading"] is not None, "At least setting one initialization method"
        #     position = [self.config["spawn_position_heading"][0][0], self.config["spawn_position_heading"][0][1]  + PGDrivableAreaProperty.SIDEWALK_NEAR_ROAD_WIDTH + PGDrivableAreaProperty.SIDEWALK_WIDTH / 2]
        #     heading = self.config["spawn_position_heading"][1]
    ...
    def _create_vehicle_chassis(self):
        ...
        # assert self.LENGTH < BaseRobotDog.MAX_LENGTH, "Vehicle is too large!"
        # assert self.WIDTH < BaseRobotDog.MAX_WIDTH, "Vehicle is too large!"
    ...
    def _add_visualization(self):
        if self.render:
            ...
            if path not in BaseRobotDog.model_collection:
                ...
                car_model.setTwoSided(False)
                BaseRobotDog.model_collection[path] = car_model
                car_model.setScale(0.3)
                # model default, face to y, translate to x
                car_model.setHpr(*HPR)
                car_model.setPos(offset[0], offset[1], offset[-1])
    ...
    def destroy(self):
        super(EgoRobotDog, self).destroy()
        if self.navigation is not None:
            self.navigation.destroy()
        self.navigation = None
        self.wheels = None
    ...
    def set_velocity(self, direction, *args, **kwargs):
        super(EgoRobotDog, self).set_velocity(direction, *args, **kwargs)
        self.last_velocity = self.velocity
        self.last_speed = self.speed

    def set_state(self, state):
        super(EgoRobotDog, self).set_state(state)
        self.set_throttle_brake(float(state["throttle_brake"]))
        self.set_steering(float(state["steering"]))
        self.last_velocity = self.velocity
        self.last_speed = self.speed
    ...
    def set_panda_pos(self, pos):
        super(EgoRobotDog, self).set_panda_pos(pos)
        self.last_position = self.position

    def set_position(self, position, height=None):
        super(EgoRobotDog, self).set_position(position, height)
        self.last_position = self.position
    ...
    def get_state(self):
        ...
        Fetch more information
        """
        state = super(EgoRobotDog, self).get_state()
        state.update(
            {
                "steering": self.steering,
                "throttle_brake": self.throttle_brake,
    ...
    def __del__(self):
        super(EgoRobotDog, self).__del__()
        # self.engine = None
        self.navigation = None
        self.wheels = None
    ...
    def detach_from_world(self, physics_world):
        if self.navigation is not None:
            self.navigation.detach_from_world()
        super(EgoRobotDog, self).detach_from_world(physics_world)

    def attach_to_world(self, parent_node_path, physics_world):
        if self.config["show_navi_mark"] and self.config["navigation_module"] and self.navigation is not None:
            self.navigation.attach_to_world(self.engine)
        super(EgoRobotDog, self).attach_to_world(parent_node_path, physics_world)
    ...
    @property
    def panda_color(self):
        c = super(EgoRobotDog, self).panda_color
        if self._use_special_color:
            color = sns.color_palette("colorblind")
            rand_c = color[2]  # A pretty green
            c = rand_c
    ...
    def convert_to_world_coordinates(self, vector, origin):
        return super(EgoRobotDog, self).convert_to_world_coordinates([-vector[-1], vector[0]], origin)

    def convert_to_local_coordinates(self, vector, origin):
        ret = super(EgoRobotDog, self).convert_to_local_coordinates(vector, origin)
        return np.array([ret[1], -ret[0]])

    @property
    def heading_theta(self):
        return wrap_to_pi(super(EgoRobotDog, self).heading_theta + np.pi / 2)
    ...
    def set_heading_theta(self, heading_theta, in_rad=True) -> None:
        ...
        :param in_rad: when set to True, heading theta should be in rad, otherwise, in degree
        """
        super(EgoRobotDog, self).set_heading_theta(heading_theta - np.pi / 2, in_rad)
        self.last_heading_dir = self.heading

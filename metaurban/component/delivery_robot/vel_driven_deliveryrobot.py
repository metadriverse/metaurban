...
class EgoDeliveryRobot(BaseObject, BaseDeliveryRobotState):
    ...
    def _preprocess_action(action):
        if action is None:
            return None, {"raw_action": None}
        action = safe_clip_for_small_array(action, -1, 1)
        VYAW_SCALE = -0.4 * 20.0 / 180 * np.pi
        VX_SCALE = 3.0
        vx =  max(0, action[0]) * VX_SCALE
        vyaw  = action[1] * VYAW_SCALE
        return np.array([vyaw, vx]), {'raw_action': (action[0], action[1])}
    ...
    def _set_action(self, action):
        dt = 2e-2 * 5
        self.set_position(self.position + action[1] * dt * np.array([np.cos(self.heading_theta), np.sin(self.heading_theta)]), height=self.HEIGHT / 2)
        self.set_heading_theta(self.heading_theta + action[0] * dt)
        return

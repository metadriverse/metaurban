...
def test_collision_with_vehicle(use_render=False):
    if not use_render:
        env = metaurbanEnv({"traffic_density": 1.0, "map": "SSS"})
    else:
        env = metaurbanEnv({"traffic_density": 1.0, "map": "SSS", "use_render": True})
    o, _ = env.reset()
    pass_test = False
    try:
...
def test_collision_with_sidewalk():
    env = metaurbanEnv({"traffic_density": .0})
    o, _ = env.reset()
    pass_test = False
    try:
...
def test_collision_info(use_render=False):
    env = metaurbanEnv({"traffic_density": 1.0, "map": "SSS", "use_render": use_render, "crash_vehicle_done": False})
    o, _ = env.reset()
    pass_test = False
    try:
        for i in range(1, 500):

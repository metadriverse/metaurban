...
if __name__ == "__main__":
    ...
    config = dict(
        ...
        sensors=dict(rgb_camera=(RGBCamera, 256, 256), depth_camera=(DepthCamera, 256, 256), semantic_camera=(SemanticCamera, 256, 256),),
    ...
    depth_back = camera.perceive(new_parent_node=env.agent.origin, position=[0, -5, 2], hpr=[0, 0, 0]).reshape(256, 256, -1)[..., -1]
    depth_top = camera.perceive(new_parent_node=env.agent.origin, position=[0, 0, 5], hpr=[0, 270, 0]).reshape(256, 256, -1)[..., -1]
    depth_side = camera.perceive(new_parent_node=env.agent.origin, position=[-5, 0, 2], hpr=[-90, 0, 0]).reshape(256, 256, -1)[..., -1]

...
from metaurban.envs.marl_envs.multi_agent_metaurban import MultiAgentmetaurban
...
def test_naive_multi_agent_metaurban():
    # env = metaurbanEnv(config={"num_agents": 1})
    # assert isinstance(env.action_space, gym.spaces.Box)
    # _step(env)
    # env.close()

    env = MultiAgentmetaurban(
        config={
            "map": "SSS",
            "num_agents": 4,
...
if __name__ == '__main__':
    test_naive_multi_agent_metaurban()

...
def test_config_consistency():
    env = metaurbanEnv({"vehicle_config": {"lidar": {"num_lasers": 999}}})
    try:
        env.reset()
...
def test_config_consistency_2():
    # env = metaurbanEnv({"map_config": {"config": "OO"}})
    the_config = 11
    env = metaurbanEnv({"map_config": {"config": the_config}})
    try:
        env.reset()
    ...
    the_config = 11
    env = metaurbanEnv({"map": the_config})
    try:
        env.reset()
        assert env.current_map.config["config"] == the_config
        assert all([v.config["config"] == the_config for v in env.maps.values()])
    ...
    the_config = "OO"
    env = metaurbanEnv({"map_config": {"config": the_config, "type": "block_sequence"}})
    try:
        env.reset()
        assert env.current_map.config["config"] == the_config
        assert all([v.config["config"] == the_config for v in env.maps.values()])

from metaurban.envs.metaurban_env import metaurbanEnv, metaurban_DEFAULT_CONFIG
from metaurban.engine.base_engine import BaseEngine
from metaurban.engine.engine_utils import get_global_config, initialize_global_config
from metaurban.utils import Config, recursive_equal
...
def test_config_sync():
    ...
    """
    try:
        env = metaurbanEnv({"vehicle_config": dict(show_lidar=False, show_navi_mark=False)})
...
def test_config_overwrite():
    ...
    The config in BaseEngine should be the same as env.config, if BaseEngine exists in process
    """
    metaurban_DEFAULT_CONFIG["map"] = "S"
    env = metaurbanEnv({"vehicle_config": dict(show_lidar=False, show_navi_mark=False)})
    try:
        env.reset()
        assert env.current_map.blocks[-1].ID == "S" and len(env.current_map.blocks) == 2
        env.close()
        env = metaurbanEnv({"map": "C", "vehicle_config": dict(show_lidar=False, show_navi_mark=False)})
        env.reset()

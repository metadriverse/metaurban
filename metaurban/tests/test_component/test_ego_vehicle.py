...
from metaurban.envs.metaurban_env import metaurban_DEFAULT_CONFIG
from metaurban.utils import Config
from metaurban.utils.coordinates_shift import metaurban_vector
...
def test_base_vehicle():
    ...
    try:
        ...
        v_config = Config(BASE_DEFAULT_CONFIG["vehicle_config"]).update(metaurban_DEFAULT_CONFIG["vehicle_config"])
        ...
        for a_x in [-1, 0, 0.5, 1]:
            for a_y in [-1, 0, 0.5, 1]:
                ...
                np.testing.assert_almost_equal(v.position, metaurban_vector(state["position"]))

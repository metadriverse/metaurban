from metaurban.envs import ScenarioEnv, metaurbanEnv
from metaurban.tests.test_functionality.test_memory_leak_engine import process_memory
...
def test_close_and_restart_metaurban_env():
    """
    Test the memory leak
    """
    start = 11
    ...
    try:
        for idx, m in enumerate([
            ...
            env = metaurbanEnv({"map": m, "use_render": False, "log_level": 50, "traffic_density": 0})
            o, _ = env.reset()
            env.close()
            memory = process_memory(to_mb=True)
            if idx == start:

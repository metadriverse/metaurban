...
class Mission {
	public:
		...
		Mission(std::string &xml_data, unsigned int agentsNum, unsigned int stepsTh, bool time, size_t timeTh,
				bool speedStop);
		...
		unordered_map<int, trajectory_dict> save_dict();


#if FULL_LOG

		bool SaveLog();
		unordered_map<int, trajectory_dict> SaveLog_dict();

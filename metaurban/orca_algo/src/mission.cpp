...
Mission::Mission(std::string &xml_data, unsigned int agentsNum, unsigned int stepsTh, bool time, size_t timeTh,
				 bool speedStop) {
	taskReader = new XMLReader(xml_data);
	agents = vector<Agent *>();
...
#if FULL_LOG
	taskLogger = new XMLLogger("log.xml", "log.xml");
	stepsLog = std::unordered_map<int, std::vector<Point>>();
	stepsLog.reserve(agentsNum);
...
unordered_map<int, trajectory_dict> Mission::save_dict() {
	unordered_map<int, trajectory_dict> traceDict;
	int j=0;
	for (auto &agentPath: stepsLog) {
		j = 0;

...
unordered_map<int, trajectory_dict> demo(string xml_data, int num)
{
    // XMLDocument doc;
    ...
    //     std::cerr << "Error parsing XML: " << error << std::endl;
    //     return resultdict;
    // }
	...
	Mission task = Mission(xml_data, num, STEP_MAX, IS_TIME_BOUNDED, TIME_MAX, STOP_BY_SPEED);
	if (task.ReadTask()) {
...
// #if FULL_LOG
		// //  std::cout << "Saving log\n";
		// task.SaveLog();
		std::unordered_map<int, trajectory_dict> resultdict = task.save_dict();
...
PYBIND11_MODULE(bind, m)
{
    ...
    m.def("demo", &demo, "A function which multiplies two numbers", py::arg("xml_data")="tmp", py::arg("num")=7);

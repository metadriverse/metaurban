...
from metaurban.component.vehicle.vehicle_type import SVehicle, LVehicle, MVehicle, XLVehicle, \
...
from metaurban.scenario.parse_object_state import parse_object_state, get_idm_route, get_max_valid_indicis
    ...
    def after_reset(self):
        ...
        for scenario_id, track in self.current_traffic_data.items():
            if scenario_id == self.sdc_scenario_id:
                continue
            if track["type"] == MetaUrbanType.VEHICLE:
                self.spawn_vehicle(scenario_id, track)
            elif track["type"] == MetaUrbanType.CYCLIST:
                self.spawn_cyclist(scenario_id, track)
            elif track["type"] == MetaUrbanType.PEDESTRIAN:
                self.spawn_pedestrian(scenario_id, track)
            elif track["type"] in [MetaUrbanType.TRAFFIC_CONE, MetaUrbanType.TRAFFIC_BARRIER]:
                cls = TrafficBarrier if track["type"] == MetaUrbanType.TRAFFIC_BARRIER else TrafficCone

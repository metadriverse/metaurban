...
from metaurban.manager.base_manager import BaseAgentManager
from metaurban.policy.idm_policy import TrajectoryIDMPolicy
from metaurban.policy.manual_control_policy import ManualControlPolicy, TakeoverPolicy, TakeoverPolicyWithoutBrake
from metaurban.policy.replay_policy import ReplayTrafficParticipantPolicy
...
class VehicleAgentManager(BaseAgentManager):
    ...
    def _create_agents(self, config_dict: dict):
        from metaurban.component.vehicle.vehicle_type import random_vehicle_type, vehicle_type
        ret = {}
...
class DeliveryRobotAgentManager(BaseAgentManager):
    """
    This class maintain the relationship between active agents in the environment with the underlying instance
    of objects.
    ...
    def __init__(self, init_observations):
        ...
        The real init is happened in self.init(), in which super().__init__() will be called
        """
        super(DeliveryRobotAgentManager, self).__init__(init_observations)
        # For multi-agent env, None values is updated in init()
        self._allow_respawn = None
        self._delay_done = None
        self._infinite_agents = None
    ...
    def _create_agents(self, config_dict: dict):
        from metaurban.component.vehicle.vehicle_type import random_vehicle_type, vehicle_type
        from metaurban.component.delivery_robot.deliveryrobot_type import random_DeliveryRobot_type, DeliveryRobot_type
        ret = {}
        for agent_id, v_config in config_dict.items():
            v_type = DeliveryRobot_type['ego']
            ...
            args = [obj, self.generate_seed()]
            if policy_cls == TrajectoryIDMPolicy or issubclass(policy_cls, TrajectoryIDMPolicy):
                args.append(obj.navigation.reference_trajectory)
            self.add_policy(obj.id, policy_cls, *args)
        return ret
    ...
    def before_reset(self):
        ...
        super(DeliveryRobotAgentManager, self).before_reset()
    ...
    def reset(self):
        ...
        self._infinite_agents = config["num_agents"] == -1
        self._allow_respawn = config["allow_respawn"]
        super(DeliveryRobotAgentManager, self).reset()

    def after_reset(self):
        super(DeliveryRobotAgentManager, self).after_reset()
        self._dying_objects = {}
        self._agents_finished_this_frame = dict()
        self.next_agent_count = len(self.episode_created_agents)
    ...
    def before_step(self):
        # not in replay mode
        step_infos = super(DeliveryRobotAgentManager, self).before_step()
        self._agents_finished_this_frame = dict()
        finished = set()
        for v_name in self._dying_objects.keys():
            self._dying_objects[v_name][1] -= 1
    ...
    def destroy(self):
        # when new agent joins in the game, we only change this two maps.
        super(DeliveryRobotAgentManager, self).destroy()
        self._dying_objects = {}
        self.next_agent_count = 0
        self._agents_finished_this_frame = dict()
...
class WheelchairAgentManager(BaseAgentManager):
    """
    This class maintain the relationship between active agents in the environment with the underlying instance
    of objects.
    ...
    def __init__(self, init_observations):
        ...
        The real init is happened in self.init(), in which super().__init__() will be called
        """
        super(WheelchairAgentManager, self).__init__(init_observations)
        # For multi-agent env, None values is updated in init()
        self._allow_respawn = None
        self._delay_done = None
        self._infinite_agents = None
    ...
    def _create_agents(self, config_dict: dict):
        from metaurban.component.vehicle.vehicle_type import random_vehicle_type, vehicle_type
        from metaurban.component.delivery_robot.deliveryrobot_type import random_DeliveryRobot_type, EgoWheelchair
        ret = {}
        for agent_id, v_config in config_dict.items():
            v_type = EgoWheelchair
    ...
    def before_reset(self):
        ...
        super(WheelchairAgentManager, self).before_reset()
    ...
    def reset(self):
        ...
        self._infinite_agents = config["num_agents"] == -1
        self._allow_respawn = config["allow_respawn"]
        super(WheelchairAgentManager, self).reset()

    def after_reset(self):
        super(WheelchairAgentManager, self).after_reset()
        self._dying_objects = {}
        self._agents_finished_this_frame = dict()
        self.next_agent_count = len(self.episode_created_agents)
    ...
    def before_step(self):
        # not in replay mode
        step_infos = super(WheelchairAgentManager, self).before_step()
        self._agents_finished_this_frame = dict()
        finished = set()
        for v_name in self._dying_objects.keys():
            self._dying_objects[v_name][1] -= 1
    ...
    def destroy(self):
        # when new agent joins in the game, we only change this two maps.
        super(WheelchairAgentManager, self).destroy()
        self._dying_objects = {}
        self.next_agent_count = 0
        self._agents_finished_this_frame = dict()
...
class VelDeliveryRobotAgentManager(BaseAgentManager):
    """
    This class maintain the relationship between active agents in the environment with the underlying instance
    of objects.
    ...
    def __init__(self, init_observations):
        ...
        The real init is happened in self.init(), in which super().__init__() will be called
        """
        super(VelDeliveryRobotAgentManager, self).__init__(init_observations)
        # For multi-agent env, None values is updated in init()
        self._allow_respawn = None
        self._delay_done = None
        self._infinite_agents = None
    ...
    def _create_agents(self, config_dict: dict):
        from metaurban.component.vehicle.vehicle_type import random_vehicle_type, vehicle_type
        from metaurban.component.delivery_robot.deliveryrobot_type import random_DeliveryRobot_type, EgoVelVehicle
        ret = {}
        for agent_id, v_config in config_dict.items():
            v_type = EgoVelVehicle
    ...
    def before_reset(self):
        ...
        super(VelDeliveryRobotAgentManager, self).before_reset()
    ...
    def reset(self):
        ...
        self._infinite_agents = config["num_agents"] == -1
        self._allow_respawn = config["allow_respawn"]
        super(VelDeliveryRobotAgentManager, self).reset()

    def after_reset(self):
        super(VelDeliveryRobotAgentManager, self).after_reset()
        self._dying_objects = {}
        self._agents_finished_this_frame = dict()
        self.next_agent_count = len(self.episode_created_agents)
    ...
    def before_step(self):
        # not in replay mode
        step_infos = super(VelDeliveryRobotAgentManager, self).before_step()
        self._agents_finished_this_frame = dict()
        finished = set()
        for v_name in self._dying_objects.keys():
            self._dying_objects[v_name][1] -= 1
    ...
    def destroy(self):
        # when new agent joins in the game, we only change this two maps.
        super(VelDeliveryRobotAgentManager, self).destroy()
        self._dying_objects = {}
        self.next_agent_count = 0
        self._agents_finished_this_frame = dict()

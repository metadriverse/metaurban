...
from metaurban.component.static_object.traffic_object import TrafficCone, TrafficBarrier
from metaurban.component.traffic_light.base_traffic_light import BaseTrafficLight
from metaurban.component.traffic_participants.cyclist import Cyclist
...
from metaurban.constants import DATA_VERSION, DEFAULT_AGENT
from metaurban.engine import get_logger
from metaurban.scenario import ScenarioDescription as SD
...
def draw_map(map_features, show=False):
    ...
    for key, value in map_features.items():
        if MetaUrbanType.is_lane(value.get("type", None)):
...
def convert_recorded_scenario_exported(record_episode, scenario_log_interval=0.1, to_dict=True):
    ...
    result[SD.METADATA] = {}
    result[SD.METADATA][SD.metaurban_PROCESSED] = True
    result[SD.METADATA][SD.ID] = result[SD.ID]
    result[SD.METADATA]["dataset"] = "metaurban"
    result[SD.METADATA]["seed"] = record_episode["global_seed"]
    result[SD.METADATA]["scenario_id"] = record_episode["scenario_index"]
    result[SD.METADATA][SD.COORDINATE] = MetaUrbanType.COORDINATE_metaurban
    ...
    tracks = {
        k: dict(
            ...
            metadata=dict(track_length=episode_len, type=MetaUrbanType.UNSET, object_id=k, original_id=k)
        )
    ...
    lights = {
        k: {
            "type": MetaUrbanType.TRAFFIC_LIGHT,
            "state": {
                ScenarioDescription.TRAFFIC_LIGHT_STATUS: [MetaUrbanType.LIGHT_UNKNOWN] * episode_len
            },
            ScenarioDescription.TRAFFIC_LIGHT_POSITION: np.zeros(shape=(3, ), dtype=np.float32),
            ScenarioDescription.TRAFFIC_LIGHT_LANE: None,
            "metadata": dict(
                track_length=episode_len, type=MetaUrbanType.TRAFFIC_LIGHT, object_id=k, dataset="metaurban"
            )
        }
    ...
    for frame_idx in range(result[SD.LENGTH]):
        ...
        for id, state in frames[frame_idx].step_info.items():
            ...
            if type == MetaUrbanType.TRAFFIC_LIGHT:
                # pop id from tracks
                if id in tracks:
                    tracks.pop(id)
                ...
                # if light_status != MetaUrbanType.LIGHT_UNKNOWN:
...
def extract_dataset_summary_and_mapping(scenario_list, dataset_name, dataset_version):
    ...
    for sd_scenario in scenario_list:
        ...
        if hasattr(SD, "update_summaries"):
            ...
            raise ValueError("Please update metaurban to latest version.")
...
def assert_scenario_equal(scenarios1, scenarios2, only_compare_sdc=False, check_self_type=True):
    ...
    for scenario_id in scenarios1.keys():
        ...
        for map_id, map_feat in old_scene[SD.MAP_FEATURES].items():
            ...
            #     old_scene_polyline = waymo_to_metaurban_vector(old_scene_polyline)
            if map_feat["type"] == MetaUrbanType.LANE_SURFACE_STREET:

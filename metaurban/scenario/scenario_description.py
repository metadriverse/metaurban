"""
A unified data format to describe a scenario that can be replayed by MetaUrban Simulator.
    ...
    scenario = {
        ...
        "tracks": {
            "vehicle1": {
                ...
                # The type string in metaurban.type.MetaUrbanType
                "type": "VEHICLE",
                ...
                "metadata": {
                    ...
                    "policy_spawn_info": {  # Information needed to re-instantiate the policy
                        "policy_class": ("metaurban.policy.idm_policy", "IDMPolicy"),
                        "args": ...,
                        "kwargs": ...,
                    }
                }
...
class ScenarioDescription(dict):
    """
    MetaUrban Scenario Description. It stores keys of the data dict.
    """
    TRACKS = "tracks"
    VERSION = "version"
    ID = "id"
    ...
    metaurban_PROCESSED = "metaurban_processed"
    TIMESTEP = "ts"
    COORDINATE = "coordinate"
    SDC_ID = "sdc_id"  # Not necessary, but can be stored in metadata.
    METADATA_KEYS = {metaurban_PROCESSED, COORDINATE, TIMESTEP}
    ...
    def sanity_check(cls, scenario_dict, check_self_type=False, valid_check=False):
        ...
        For metadata, it should at least contain metaurban_processed, coordinate and timestep.
    ...
    def _check_map_features(cls, map_feature):
        ...
        for id, feature in map_feature.items():
            if MetaUrbanType.is_lane(feature[ScenarioDescription.TYPE]):
    ...
    def _check_object_state_dict(cls, obj_state, scenario_length, object_id, valid_check=True):
        ...
        # Check type
        assert MetaUrbanType.has_type(obj_state[cls.TYPE]
                                      ), "MetaUrban doesn't have this type: {}".format(obj_state[cls.TYPE])

        # Check set type
        assert obj_state[cls.TYPE] != MetaUrbanType.UNSET, "Types should be set for objects and traffic lights"
    ...
    def map_height_diff(map_features, target=10):
        ...
        for feature in map_features.values():
            if not MetaUrbanType.is_road_line(feature[ScenarioDescription.TYPE]):
                continue

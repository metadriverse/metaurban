...
def draw_top_down_map_native(
    ...
    if semantic_map:
        ...
        for obj in all_lanes.values():
            if MetaUrbanType.is_lane(obj["type"]) and not draw_center_line:
                pygame.draw.polygon(
                  ...
                  or (MetaUrbanType.is_lane(obj["type"]) and draw_center_line)):
                ...
                for index in range(0, len(obj["polyline"]) - 1, points_to_skip):
                    color = [255, 0, 0] if MetaUrbanType.is_lane(obj["type"]) and index==0\

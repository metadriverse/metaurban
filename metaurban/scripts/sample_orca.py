# 在random_start_and_end_points得到随机的起始点和终点之后，对于随机生成的agents进行路径规划
# 1. 可在line268 更改目标xml文件名
# 2. generate_template_xml用于生成orca所需要的xml文件
# 3. get_planning将利用start/goal 以及xml文件进行路径规划，最终结果保存在nexts中。‘nexts的数据结构是长度为N-steps的list，每个step保存了（X个agents当前的x，y坐标）’
# 4. line277-279 是为了生成可视化动画而对nexts进行转换，（该步骤不改变生成结果）
# 5. line282之后是可视化代码
...
# self.planning = OrcaPlanning("./metaurban/orca_algo/task_examples_demo/custom_road_template.xml")

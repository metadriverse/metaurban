...
if __name__ == "__main__":
    ...
    os.makedirs('./IL_data', exist_ok=True)
    os.makedirs(f'./IL_data/seed_{env.current_seed:06d}', exist_ok=True)
    assert predefined_config['ManualControl'], 'please set manual control to True to collect data'
    try:
        print(HELP_MESSAGE)
        for i in range(1, 1000000000):
            o, r, tm, tc, info = env.step([0., 0.0])   ### reset; get next -> empty -> have multiple end points
            action = info['action']
            np.save(f'./IL_data/seed_{env.current_seed:06d}/time_{i-1:05d}.npy',
                    {'obs': o, 'action': action, 'reward': r, 'terminal': tm, 'trunc': tc, 'info':info})
            if (tm or tc):
